<?php 
    ...
    if ($searchNome == '') {
        $sql = "SELECT * FROM alunosInativos ORDER BY id";
        $stmt = $conexao->prepare($sql);
    } else {
        $sql = "SELECT * FROM alunosInativos WHERE nome LIKE ?
                OR endereco LIKE ?
                OR bairro LIKE ?
                OR doc LIKE ?
                OR telefone LIKE ?
        ...
        $stmt->bind_param("ssssss", $searchNome, $searchNome, $searchNome, $searchNome, $searchNome, $searchNome);
    ...
    $linhas = "SELECT COUNT(*) AS total FROM alunosInativos";
    ...
    <main id="main">
        <div class="content">
            <div class="search">
                ...
                <input type="text" name="searchInputInativo" id="searchInputInativo">
                <button id="searchInativo">Procurar</button>
            </div>
            <div class="lista">
                <table class="table">
                    <thead>
                    ...
                    <tbody>
                        <?php  
                            while ($user_Data = mysqli_fetch_assoc($result)) {
                                ...
                                $dataDeSaida = $user_Data['dataDeSaida'] ?? "";
                                if ($dataDeSaida) {
                                    $dataFormatada = (new DateTime($dataDeSaida))->format('d/m/Y');
                                } else {
